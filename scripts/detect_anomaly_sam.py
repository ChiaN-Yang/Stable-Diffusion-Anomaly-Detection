...
class AnomalyDetector:
    ...
    def detect(self, folder_path, file_name, results_dir):
        ...
        coordinates = peak_local_max(diff_inv, min_distance=20)
        diff_inv_cir = cv2.cvtColor(diff_inv, cv2.COLOR_GRAY2RGB)
        for point in coordinates:
            diff_inv_cir = cv2.circle(diff_inv_cir, point, radius=5, color=(0, 0, 255), thickness=-1)
        cv2.imwrite(str(results_dir/f"thresh_{file_name}"), diff_inv_cir)
        ...
        cv2.imwrite(str(results_dir/f"mask_{file_name}"), mask_data)
        return sum(diff_inv.flatten()), sum(diff_inv.flatten()), score, sum(mask_data.flatten())


if __name__ == "__main__":
    detector = AnomalyDetector("/mnt/d/reconstruct/Old/VisA/visa_test/", "/mnt/d/results/", "visa")
