...
class PixelResultsToCSV:
    ...
    def get_score_list(self, folder_path: str, categorys: List[str]):
        ...
        for category in categorys:
            auroc, f1= self.read_all_images_f1_score(os.path.join(folder_path, category), category)
    ...
    def read_all_images_f1_score(self, folder_path, category):
        ...
        gt_folder = f"/mnt/d/dataset/MVTec/{category}/ground_truth/**/*.png"
        for gt_path, file_path in zip(glob.glob(gt_folder, recursive=True), glob.glob(f"{folder_path}/**/real_mask_*", recursive=True)):
...
if __name__ == '__main__':
    for strength in [0.1, 0.2, 0.5, 0.7]:
        PixelResultsToCSV(result_path='/mnt/d/results/sam_pro/', strength=strength, dataset='mvtec')
